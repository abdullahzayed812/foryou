...
import { useFulfillerDisputes } from "@/features/disputes/hooks";
...
export function FulfillerDisputesPage({
  basePath,
  detailBase,
}: {
  ...
  const { data, isLoading } = useFulfillerDisputes(basePath);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        {data?.map((dispute) => (
          <Link key={dispute.id} to={`${detailBase}/${dispute.id}`}>
